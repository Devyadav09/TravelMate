...
const rentalProviderSchema = new Schema(
    {
        agencyName: {
            type: String,
            required: true,
...
rentalProviderSchema.pre("save", async function (next) {
...
rentalProviderSchema.methods.isPasswordCorrect = async function(password){
    return await bcrypt.compare(password,this.password)
}

// make the custom method for the get the ACCESS TOKEN

rentalProviderSchema.methods.generateAccessToken = function(){
    return jwt.sign(
        {
            _id : this._id,
            email : this.email,
...
rentalProviderSchema.methods.generateRefreshToken = function(){
    return jwt.sign(
        {
            _id : this._id
        },
